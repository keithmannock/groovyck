...
is passed from one Java virtual machine to another Java virtual machine. 
Rather than making a copy of the implementation object in the receiving Java 
virtual machine, RMI passes a remote stub for a remote object. 
The stub acts as the local representative, or \emph{proxy}, 
for the remote object and basically is, 
to the client, the remote reference. 
The client invokes a method on the local stub, which is responsible for carrying out 
the method invocation on the remote object.

To be able to call methods on objects that do no exist on
...
sends them to the server through the network (a process known as \emph{marshalling}. 
The server receives
...
serialising objets as parameters, recovering from network errors, etc)
...
folder as the main class is. A basic example of such a file might
have the following aspect: 

\VerbatimInput{src/client.policy}
%
This file would allow code on this machine to accept connections from
machines on the local host, i.e.~the machine the program is running on) on
any port. 
...
\VerbatimInput{src/EchoService.java}
%
...
\VerbatimInput{src/EchoServer.java}
%
We can now compile the server. The stub is generated dynamically for us using a proxy and 
therefore we don't get to see that code. Therefore compiling 
the server requires a normal Java compilation: 
...
\VerbatimInput{src/EchoServerLauncher.java}
%
...
to serialise and marshall the objects sent and received to the
...
\item Write a launcher that binds the server to the registry, and execute it. 
\end{enumerate}
